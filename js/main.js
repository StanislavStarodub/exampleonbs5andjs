...
document.querySelector('button.add_new').addEventListener('click', function (e) {
  ...
  let amount = document.getElementById('good_amount').value;
  if (name && price && amount) {
    document.getElementById('good_name').value = '';
    document.getElementById('good_price').value = '';
    document.getElementById('good_amount').value = 1;
    let goods = JSON.parse(localStorage.getItem('goods'));
    goods.push(['good_' + goods.length, name, price, amount, 0, 0, 0]);
    ...
    for (let i = 0; i < goods.length; i++) {
      tbody.insertAdjacentHTML('beforeend',
        ...
        <tr class="align-middle">
          ...
          <td>${goods[i][3]}</td>
          <td>
          <button class="good_delete btn-danger" data-delete="${goods[i][0]}">&#10006;
          </button>
          </td>
          <td>
          <button class="good_delete btn-primary" data-goods="${goods[i][0]}">&#10149;
          </button>
          </td>
        </tr>
